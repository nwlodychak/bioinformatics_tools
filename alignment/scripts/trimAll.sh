...
mkdir -p data/trimmed $pairedOutPath $unpairedOutPath
...
function trimAll {
    ...
    do
        ...
        $pairedOutPath$sampleName.paired$leftSuffix \
        $unpairedOutPath$sampleName.unpaired$leftSuffix \
        $pairedOutPath$sampleName.paired$rightSuffix \
        $unpairedOutPath$sampleName.unpaired$rightSuffix \
        HEADCROP:0 \
        ILLUMINACLIP:$PATH_TO_TRIMMOMATIC/adapters/TruSeq3-PE.fa:2:30:10 \
        LEADING:20 TRAILING:20 SLIDINGWINDOW:4:30 MINLEN:36

        printf "$sampleName trimmed successfully. Outputs:\
            \n\t$pairedOutPath$sampleName.paired$leftSuffix \
            \n\t$unpairedOutPath$sampleName.unpaired$leftSuffix \
            \n\t$pairedOutPath$sampleName.paired$rightSuffix \
            \n\t$unpairedOutPath$sampleName.unpaired$rightSuffix"
